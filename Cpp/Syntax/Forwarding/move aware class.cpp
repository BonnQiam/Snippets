...
public:
    // following values are static, so they are shared by all instances of MyString, used to track the number of constructor, copy constructor, assignment operator, move constructor, move assignment operator and destructor
    static size_t DCtor;    // default-ctor, tor mans "constructor"
...
namespace std {
    ...
    // 为了使得 MyStringNoMove 和 MyString 可以作为 unordered_map 的 key, 需要定义 hash
    template<>
...
void test_moveable(M c1, NM c2, long &value) {
    // M 为 moveable 类型, NM 为 non-moveable 类型
...
int main() {
    long value = 3000;           // vector 测试结果的 MCtor与 CCtor 结果大于3000000,是因为vector的动态扩容,当容量不够的时候,会动态分配并进行拷贝。
