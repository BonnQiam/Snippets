...
{
public:
    // non-explicit-one-argument ctor 可以把别的东西转换为 Fraction 这种
...
{
public:
    // explicit-one-argument ctor 可以把别的东西转换为 Fraction 这种
...
{
    Fraction f(3, 5);
    Fraction ff = f + 3; // 会将 3 转换为(3,1)   隐式转换
    ff.getNumAndDen();

    // 如果不想编译器进行隐式转换,可以在前面添加 explicit 关键字

    // c++2.0之前 explicit 只禁止"存在单一实参"转换
    // c++2.0之后 explicit 可以进制"多个实参"转换
    //    Fraction1 f1(3,5);
